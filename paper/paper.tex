...
Malacology
includes a non-exhaustive set of
...
abstractions when used to construct the Mantle and ZLog services. First, we
benchmark scenarios where contention is at the sequencer by examining the
interfaces used to map ZLog onto Malacology; specifically, we describe the
sequencer implementation and the propagation of object \& data interfaces
...
and physical design implementation trade-offs.  Since this work
...
\subsection{Load Balancing Multiple ZLog Sequencers with Mantle}
...
resources, cache roherence, and managing multiple sequencers.
...
partitioning load) to (1) alleviate load from overloaded servers with multiple
sequencers and (2) recover from failure.

\begin{figure}[t!]
\centering
\includegraphics{figures/mantle-balancer-behaviors.png}
\caption{[\href{https://github.com/double-blind-submitter/osdi16}{source}] The
load balancing interface alleviates overloaded servers in environments with multiple sequencers in
...
\caption{[\href{https://github.com/double-blind-submitter/osdi16}{source}]
}\label{fig:mantle-balancer-performance}
\end{figure}
...
server.  Instead of measuring contention at the clients like we did in
Section~\ref{sec:sequencer-implementation}, these experiments create contention
at the sequencers by setting the capability infrastructure to ``round trip
mode". In this mode clients go to the sequencer for every request. We
implement this by piggybacking on \texttt{fstat()} and using the shared
resource interface to force round trips.  Because the sequencer hands out
positions for the tail of the log, the workload is read-heavy.
...
and the ``Mantle" curve shows the throughput of a custom load balancer we wrote
specifically for sequencers. The increased throughput for the ``CephFS" and
``Mantle" curves between 0 and 60 seconds are a result of migrating the
sequencer(s) off of overloadeed servers. In addition to showing that migrating
sequencers improves performance, Figure~\ref{fig:mantle-balancer-behaviors}
also demonstrates features that we will explore in the rest of this section.
Section~\ref{sec:feature-balancing-modes} and~\ref{sec:feature-migration-units}
shows why the performance of Mantle is better than CephFS when the cluster
stabilizes at time 100 seconds and Section~\ref{sec:feature-backoff} explains
the slope and location of the rebalancing phase between 0 and 60 seconds by
comparing the aggressiveness of the CephFS and Mantle balancers.
...
the subsystems that handle client requests or forwarded requests in different
ways. In Figure~\ref{fig:mantle-balancer-behaviors}, the ``CephFS" curve shows
the performance of the balancing mode that CephFS uses {\it most of the time}.
...
is a problem as it prevents administrators from properly exploring the
balancing state space.

Mantle gives the administrator more control over the balancing policies; for
the ``Mantle" bar in Figure~\ref{fig:mantle-balancer-performance} we use the
load balancing interface to program logic for balancing read-heavy workload
into the load balancing service. The resulting performance is more stable and
performs better because we created our own mode for balancing sequencers.

We have identified two balancing modes that are relevant for making migration
decisions for sequencers. Using Mantle, the administrator can put the load
balancing service into ``proxy mode" or ``sequencer mode". In ``proxy mode" one
server receives all requests and farms off the requests to slave servers.  In
``sequencer mode" clients go directly to the server that has their sequencer.
These modes are illustrated in Figure~\ref{fig:mantle-modes}. ``None" is when
all sequencers are co-located on one physical server -- performance for that
mode is shown by the ``No Balancing" curve in
Figure~\ref{fig:mantle-balancer-behaviors}. ``Sequencer" mode in the lower left
shows how clients for sequencer 2 are re-directed to server B. The two ``Proxy"
mode diagrams on the right show how clients continue sending reqeusts to server
A even though sequencer 2 has migrated to server B (the difference between
``Proxy (Half)" and ``Proxy (Full)" is discussed in the next section).
...
Figure~\ref{fig:mantle-mode-performance} shows the throughput (\(y\) axis) over
...
in Figure~\ref{fig:mantle-mode-performance}(a) start at less than 1000
ops/second and at time 60 seconds Mantle migrates Sequencer 1 to the slave
server. Performance of Sequencer 2 decreases because it has stayed on the proxy
which now handles all client requests, processes requests for Sequencer 2, and
forwards requests for Sequencer 1 to the slave server. Sequencer 1 performance
improves dramatically because lookups for that sequencer are parallelized with
the operations on server 1. Cluster throughput improves at the cost of
decreased throughput and unfair throughput shares for Sequencer 2.
Figure~\ref{fig:mantle-mode-performance}(b) is set to sequencer mode manually
(no balancing phase) and shows that performance is worse than proxy mode {\it
and} unfair amongst sequencers. In this case, the scatter-gather process used
for cache coherence in the metadata protocols causes strain on the server
housing Sequencer 2 resulting in uneven performance. 

Intuitively, sequencer mode is a superior balancing mode and should perform
better; after all, this is the best strategy for distributing POSIX metadata in
write-heavy workloads~\cite{ren:sc2014-indexfs,
patil:fast2011-giga,sevilla:sc15-mantle}. But our experiments show that proxy
mode is superior for the sequencer's read-heavy workload.  In this case,
separating the handling of the client request and doing the actual operation
improves performance. Processing client requests and doing the actual operation
is too heavy weight for one server and sequencers on slave nodes can go faster
because work is split up; the proxy processes the client request and the slave
finds the tail of the log and delivers the answer.  This phenomenon is not
...
How much load to try and migrate is not a tunable parameter in the original
CephFS balancers. The most useful way to express how much load to send is as a
fraction of some other metric such as total load. Expressing this heuristic is
not easily achievable with outward facing tunable parameters (i.e. system
knobs) but can be expressed with programmability quite easily. To force the
load balancer into ``Proxy (Half)" scenario in Figure~\ref{fig:mantle-modes},
which sets the balancer to proxy mode with migration units of half the load on
the current server we can use the Mantle policy:\\
...
``Proxy (Full)"` setup in Figure~\ref{fig:mantle-modes}, one would use:\\
...
"Overloaded" environment).}\label{fig:mantle-modes}
\end{figure}
...
sequencer mode does not perform as well for read-heavy workloads and we even
see a throughput improvement when migrating all load off the first server,
leaving the first server to do administrative tasks (this is actually common in
the metadata cluster because the ``first" server does a lot of the cache
coherence work) while the second server does all the client work. Proxy Mode
does the best in both cases and shows large performance gains when completely
decoupling client request handling and operation processing in Proxy Mode
(Full). 

The parameter that controls the migration units helps the administrator control
the sequencer or co-location or distribution across the cluster. This trade-off
was explored extensively in the Mantle paper and warrants further exploration
as all our experiments indicate a rich set of states to explore.

\subsubsection{Feature: Backoff Paramters}
\label{sec:feature-backoff}

The balancing phase from 0 to 60 seconds in
Figure~\ref{mantle-balancer-behaviors} shows different degrees of
aggressiveness in making migration decisions; CephFS makes a decision quickly
and throughput jumps 2.5K ops/second within 10 seconds while Mantle takes a
more time to stabilize. This conservative behavior is controlled by programming
the balancer to (1) use different conditions for when to migrate and (2) use a
threshold for sustained overload. \texttt{when()} is part the Mantle API and
the callback expects to return a true or false, which corresponds to whether to
migrate or not. For the Mantle curve in Figure~\ref{mantle-balancer-behaviors}
we set the \texttt{when()} policy to wait for load on the receiving server to
reach 0.1. At the beginning of the run the load of the receiving server spikes
because requests for cache-coherence start as the sequencers are created. It
takes 60 seconds for this load to drop to an acceptable level. Also, the
migration phase is a little more conservative in sending load; the Mantle curve
in Figure~\ref{mantle-balancer-behaviors} takes longer to reach peak throughput
because we do the first migration and {\it wait to see how it affects the
system}; seeing that after migration the third server is still underloaded
allows the balancer to further improve performance by migrating a sequencer to
the laster server.

The other way to change aggressiveness of the decision making is to program
into the balancer a policy that waits a certain number of iterations after a
migration before proceeding. In Mantle, the policy would use the save state
function to do a countdown after a migation:
...
Behavior graphs and performance numbers for this feature is omitted, as the
throughput graphs are as expected: the more conservative the approach the less
overall throughput. 
...
sequencers on the same server, load balancing a sequencer per physical server
shows a 1.8\(\times\) throughput improvement.\\

\noindent\textbf{Features:} different load balancing modes control forwarding
vs. client redirection, tuning the backoff parameter controls conservative vs.
aggressive decision making, and tuning load migration unit parameters controls
sequencer distribution vs. co-location.\\

...
abstractions used to construct the Mantle and ZLog services. We also discuss
latent capabilities we discovered in this process that lets us navigate
different trade-offs within the services themselves. First, we benchmark
scenarios where contention is at the sequencer by examining the interfaces used
to map ZLog onto Malacology; specifically, we describe the sequencer
implementation and the propagation of object \& data interfaces interfaces.
Next, we benchmark scenarios when there are multiple logs by using Mantle to

...
% todo:
%  - ellude to the need for policies to be avialable quickly to support
%    our experiment about propogtation delay
%
...
% todo:
%  - ellude to the need for interfaces to be avialable quickly to support
%    our experiment about propogtation delay
%
...
a distributed metadata service with dynamic subtree ipartitioning.
...
to achieve high throughput network counter service (called a \emph{sequencer})
that avoids all persistent I/O in the common case.

While a full description of the CORFU system is beyond the scope of this
paper, we'll briefly describe the storage interface, sequencer, and recovery
protocol that form the core components of CORFU that we implement within
Malacology:

{\bf Sequencer.}
High-performance in CORFU is achived using a sequencer service that assigns
client log positions by reading from a volatile, in-memory counter which can
run at a very high throughput and at low-latency. Since the sequencer is
centralized, ensuring seiralizability in the common case is trivial.  The
primary challenge in CORFU is handling the failure of the sequencer in a way
that guarantees correctness.  Failures of the sequencer service in CORFU are
...
log and invalidate stale client requests using an epoch-based protocol.

{\bf Storage Interface.} The storage interface is a critical component in the
CORFU design. Storage devices must provide a \emph{write}-once, random
\emph{read} interface for reading and writing log entries. The key to
correctness in CORFU lies with the enforcement of up-to-date epoch tags on
client requests; requests tagged with out-of-date epoch values are rejected,
and clients are expected to request a new tail from the sequencer after
refreshing state from an auxillary service. This mechanism forms the basis for
sequencer recovery.
...
\subsection{Interface Installation}

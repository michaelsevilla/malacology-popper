...
\usepackage{color}
\usepackage{comment}
\includecomment{newcomment}
\includecomment{oldcomment}
...
\nc{New changes are in red}

\oc{Old prose is in blue}

%\nc{
%\begin{enumerate}
%  \item clarify what was implemented: diagram of interfaces and prose specifying the difference between Malacology and prior work (reviewers B, C, D, E)
%  \item describe what makes a programmable storage system (reviewers C, D)
%  \item organize motivation: group interfaces into categories (e.g., core functionality, features, and performance optimizations) and describe other applications we can build with Malacology (reviewers B, C)
%  \item clarify terminology: service vs. file system metadata, storage vs. file system, dirty vs. clean slate approaches, soft-state caching (reviewers B, C)
%  \item address security, access control, and how it affects composability/multi-tenancy on correctness (reviewer D)
%\end{enumerate}
%}
\end{newcomment}
...
\begin{oldcomment}
\oc{. Malacology is both a prototype for a programmable storage system and a}
\end{oldcomment}
\begin{newcomment}
\nc{, our prototype programmable storage system. It uses the programmable storage}
\end{newcomment}
design approach to evolve
...
Load Balancing
  ...
  \\
\end{tabular}
\caption{Common internal abstractions. The same ``internal abstractions" are common across large-scale
...
italicized abstractions} are interfaces discussed in this paper.}
...
\begin{newcomment}
\nc{and Figure~\ref{fig:implementation-overview} compares what was already
present in Ceph (gray boxes) to the Malacology interfaces we added (blue
trapezoids); Section~\ref{sec:services} will describe the higher-level
services (pink arrows) we built with these interfaces.}

\begin{figure}[tbp]
\centering
\includegraphics[width=100mm]{figures/implementation-overview.png}
\caption{\nc{Malacology is implemented on the daemons and clients that run in a
Ceph cluster. The interfaces expose internal subsystems of the Ceph modules and
are used as building blocks for higher-level services.}
...
\begin{oldcomment}
\oc{The distributed metadata service in Ceph provides clients with a POSIX file
system abstraction~\cite{weil:sc2004-dyn-metadata}.}
\end{oldcomment}
\begin{newcomment}
\nc{File systems provide clients with the familiar POSIX file abstraction.
...
\nc{\\ \noindent\it{\textbf{Ceph Implementation: }}}
\end{newcomment}
In Ceph, the locking service implements a
...
While designed for the file abstraction, indexing, locking, and caching are
...
and expose a POSIX file interface. Applications that manage large amounts of
...
policies. We will show how this is used in Section~\ref{sec:seq} when we discuss a
...
Composing Mantle with the service metadata and durability interfaces, Mantle
...
load balancing policies can express a variety of rules such as multi-tenant

...
ZLog sequencer that uses the Malacology load balancing interface enjoys the
load balancing features of the of the metadata cluster. The ``Sequencer
Baseline'' has no background sequencers, while the rest of the curves show the
throughput of 1 sequencer when an additional sequencer is running in the
background generating load.}\label{fig:mantle-seq-thruput}
\end{figure}

\subsubsection{Load Balancing Sequencers}

We evaluate the load balancing interface by showing the performance
implications of shedding load. The experiments use our internal cluster which
uses 10 nodes to store objects, one node to monitor the cluster, and 3 nodes
that can accomodate sequencers. The workload is two logs, one with 7 clients
and the other with 8 clients; we chose this workload because this load is the
...
balancer aggressively sheds half its load to its neighbor as soon as it can.
It is based off the ``Greedy Spill Even" approach in the
Mantle~\cite{sevilla:sc15-mantle} paper but is reproduced in
Figure~\ref{listing:greedy-spill} for the reader's convenience. 
...
server immediately because it uses a policy designed for bursty workloads. This
is a trace of the same workload running twice: once with a single host server and once
with Mantle.}
...
time (\(x\) axis).  ``Sequencer Baseline'' is the performance of one sequencer
...
sequencer on an underutilized physical server.  ``No Load Balancing" adds a
...
are the throughput of one of the sequencers when a background sequencer is
running. These policies differ by the threshold at which they start balancing:
``Sequencer Balancing Policy" starts balancing immediately and ``Conservative
Balancing Policy" waits for 120 seconds before migrating sequencers.

The variation of the sequencer throughputs helps us determine which sequencers
are on overloaded physical servers.  ``Sequencer Baseline" is the best
...
spike at the end like the other curves.  ``No Load Balancing" shows an
overloaded sequencer because the performance of the monitored sequencer is low
and unstable (high variation in performance). We also deduce that it is
overloaded by comparing to the ``Sequencer Balancing Policy" and ``Conservative
Balancing Policy" curves, which show improved performance when load is
migrated. 
...
has been partitioned into the two cluster setup stages. The CPU utilization is
confined to one core because load balancer interface is not multi-threaded.
Recall the the load balancing interface re-uses the POSIX metadata cluster
migration mechanisms and as it has been shown that metadata performance does
not improve with multiple threads~\cite{konstantinos:pdsw2014-lustre-metadata}. 

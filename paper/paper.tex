...
installed on the storage devices in the system, supports the evolution of
...
in a large cluster. In In practice the costs are, in addition to cluster-wide
propagation of interface updates, includes the network round-trip to the
interface management service, the Paxos commit protocol, and other factors
...
migrate. We defined a  migration unit as the amount of load to try and migrate.
...
be expressed quite easily. To force the load balancer into the Proxy Mode
(Half) scenario in Figure~\ref{fig:mantle-modes}, which sets the balancer to
proxy mode with migration units of half the load on the current server we can
use the Mantle policy:\\

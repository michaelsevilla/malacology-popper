...
service by embedding a Lua VM in the monitors that implements a generic API
for creating dynamic interfaces around existing subsystems that may associated
additional state with a subsystem map.
...
defining new file types allowing the indexing service to be repurposed for
...
not enforced by the runtime, the system allows injectable strings through the
...
utilizing the metadata service as a sequencer, and propogating interface

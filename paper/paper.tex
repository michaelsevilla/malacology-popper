...
servers and clients.
Ceph provides object storage via 
RADOS~\cite{weil_rados_2007}, the reliable distributed object store. 
...
dynamic environment, resulting in an object storage API with economy of 
...
\paragraph*{Malacology:} Metadata service policies and object storage interfaces are stored durably
...
We use capability policies in the metadata service to tune the amount of
...
maintains its capability lease. In throughputs labeled $2C,<time>$ we show
...
related to reliability. We propose that systems expose their services in
...
one point along that path in which we have looked at a target

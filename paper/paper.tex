...
\autocite{sevilla:sc15-mantle}). To succesfully tune the system, a
...
is left off the table.  Starfish \autocite{herodotou:cidr2011-starfish} made
an attempt of this for Hadoop but this technique would need to severely limit
the space of parameters in order to feasible, similar to the approach used in
\autocite{behzad:sc2013-autotuning}.
...
is the work that focused on HDFS scalability for metadata-intensive workloads
...
proof-of-concept active storage implementation.  Figure
...
burden for users. Rather it appears that a gap in existing interfaces are
...
~\cite{weil_rados_2007}, the reliable distributed object store. RADOS

...
system that allows existing abstractions to be re-used and composed to form
...
\item An interface for **durability** that persists policies using the underyling storage
...
Hadoop's jobs, and uses techniques like exposing data locality and relaxing POSIX constraints to achieve
...
existing interfaces are being addressed through programmability. In fact,
...
%Thus, system
%services realized through extensibility should be versioned together as a
%group, exposing the version to allow introspection and domain-specific
%treatment of version mismatch scenarios.

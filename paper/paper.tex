...
\caption{Malacology is implemented on the daemons and clients that run in a
Ceph cluster. The gray callout bubbles show where each interface is in the
storage stack and the Ceph modules, functions, and/or variables we modified.
...
prototype via Lua~\cite{ierusalimschy1996lua} bindings and
Figure~\ref{fig:implementation-overview} shows where each interface lives in
the storage stack.

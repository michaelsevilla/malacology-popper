...
and includes membership details, hardware layout (e.g., racks, power
supplies, etc.), data layout, and daemon state and configuration. It differs from
...
we are referring to the daemon(s) that manages file system metadata (not service metadata).}
\addressesissue{4}
...
\noindent{\it{\textbf{Malacology: }}} we expose a strongly-consistent
...
because it lets daemons come to a consensus on system critical state. Bugs in the
internal subsystems or omitting this from services that need this type of
...
\oldcomment{\noindent 4.2 Object I/O Interfrace}
...
\noindent{\it{\textbf{Existing Ceph Implementation: }}} provides storage

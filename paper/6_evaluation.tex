...
The sequencer is implemented as a custom file type in which the sequencer state
(a 64-bit integer) is embedded in the inode of a file representing the
...
experience uncontended access for a longer period of time.
...
monitors using HDD-based storage, we were able to decrease this interval to an
average of 222 ms.
...
time}.  CephFS has 3 modes for balancing load: CPU mode, workload mode, and
